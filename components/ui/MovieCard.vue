...
const addWithStatus = async (status) => {
  try {
    isSaving.value = true
    await addToWatchlist(props.movie.id, 'movie', status)
...
const onSaveRate = async ({ rating, comment }) => {
  await rateContent({ tmdbId: props.movie.id, contentType: 'movie', rating, comment })
  rateOpen.value = false
}

// Kartın tamamı zaten tıklanabilir; ekstra info ikonu kaldırıldı
</script> 
